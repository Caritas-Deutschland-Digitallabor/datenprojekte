...
def analyze_column(column_name, df, model):
    ...
    for index, row in df.iterrows():
        if pd.notna(row[column_name]):
            terms = [
                term.strip() for term in row[column_name].split(",") if term.strip()
            ]
...
print("=== Initializing ===")
df = pd.read_csv(
    "/Users/ramius/Desktop/CodeVault/Caritas Datenprojekt/datenprojekte_git/MarkdownConverter/data/csv/combined_all_projects.csv",
    sep=";",
)
...
for column_name in columns_to_analyze:
    all_terms, unique_terms, term_counts, embeddings = analyze_column(
        column_name, df, model
    )
...
def find_optimal_clusters(embeddings, unique_terms, max_k=15):
...
def reassign_noise_to_clusters(embeddings, labels):
    ...
    for idx in noise_indices:
        distances = [
            np.linalg.norm(embeddings[idx] - centroid)
...
def perform_clustering_analysis(column_name, unique_terms, embeddings, term_counts):
    ...
    print(
        f"Final clustering: {len(clusters)} clusters, all {len(unique_terms)} terms assigned"
    )
...
Antworte nur mit dem Kategorienamen auf Deutsch, nichts anderes."""
            ...
            try:
                print(
                    f"🤖 Asking Ollama about cluster {cluster_id}: {terms_text[:50]}..."
                )
                ...
                if response.status_code == 200:
                    ...
                    if (
                        not cluster_name
                        or len(cluster_name.split()) > 4
                        or cluster_name.lower().startswith("category")
                        or len(cluster_name) > 30
                    ):
                        print("⚠️  Invalid response, using fallback")
                        cluster_name = (
                            cluster_terms[0] if cluster_terms else f"Group_{cluster_id}"
                        )
                    else:
        ...
        print(
            f"🏷️  Cluster {cluster_id}: '{cluster_name}' <- {', '.join(cluster_terms[:3])}{'...' if len(cluster_terms) > 3 else ''}"
        )
...
def create_visualization(
    ...
    for i, (cluster_id, cluster_terms) in enumerate(clusters.items()):
        ...
        plt.scatter(
            ...
            c=[colors[i]],
            label=f"{cluster_name} ({len(cluster_terms)})",
            alpha=0.8,
            s=80,
            edgecolors="black",
            linewidth=0.5,
    ...
    plt.tight_layout()
    plt.savefig(
        f"term_clustering_{column_name.lower()}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def export_results(column_name, unique_terms, best_labels, cluster_names, term_counts):
    ...
    results_df.to_csv(f"term_clustering_{column_name.lower()}_results.csv", index=False, sep=";")
    ...
    summary_df.to_csv(f"term_clustering_{column_name.lower()}_summary.csv", index=False, sep=";")
...
for column_name in columns_to_analyze:
    ...
    clusters, cluster_names, best_labels, best_method = perform_clustering_analysis(
        column_name, data["unique_terms"], data["embeddings"], data["term_counts"]
    )
