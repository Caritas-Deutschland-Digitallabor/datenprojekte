...
class MarkdownCreatorProjects:
    ...
    def load_csv_file(self, data_file):
        """Load csv data files."""
        try:
            return pd.read_csv(f"TermSimilarity/{data_file}.csv", delimiter=";")
    ...
    def _replace_placeholder_with_value(self, content, placeholder, value):
        ...
        if placeholder_name in {"cluster_name"}:
    ...
    def create_multiple_files(self):
        ...
        for index, row in self.data_content.iterrows():
            ...
            if "cluster_name" in self.data_content.columns and pd.notna(row["cluster_name"]):
                filename = str(row["cluster_name"]).replace(" ", "_").replace("/", "_")
...
#%%
creator = MarkdownCreatorProjects("Art", "term_clustering_art_summary")
creator.create_md_file("test", row_index=1)
creator.save_file()
#creator.create_multiple_files()

#%%
creator = MarkdownCreatorProjects("Einsatzbereich", "term_clustering_einsatzbereich_summary")
creator.create_md_file("test", row_index=1)
creator.save_file()
#creator.create_multiple_files()
