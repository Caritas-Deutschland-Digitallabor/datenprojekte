...
class OrganizationLinkFinder:
                ...
                for row in reader:
                    ...
                    if org_field:
                        ...
                        for org_name in org_names:
                            if org_name and org_name not in [org['name'] for org in self.organizations]:
    ...
    def search_duckduckgo_html(self, org_name: str, retry_count: int = 0) -> List[Dict]:
        ...
        try:
            ...
            if retry_count < 2:
                ...
                print(f"    Request error, waiting {wait_time:.1f} seconds before retry {retry_count + 1}/2...")
    ...
    def fallback_search(self, org_name: str) -> List[str]:
        ...
        if len(org_words) >= 2:
            ...
            if len(acronym) >= 2:
                potential_domains.extend([f"{acronym}.org", f"{acronym}.com", f"{acronym}.gov"])
        
        # Try full name variations
        full_name = ''.join(org_words)
        if len(full_name) > 3:
            potential_domains.extend([f"{full_name}.org", f"{full_name}.com", f"{full_name}.gov"])
        
        # Try hyphenated version
        if len(org_words) > 1:
            hyphenated = '-'.join(org_words)
            potential_domains.extend([f"{hyphenated}.org", f"{hyphenated}.com", f"{hyphenated}.gov"])
    ...
    def analyze_results_with_llm(self, org_name: str, results: List[Dict]) -> Optional[str]:
...
Consider the title and description context to make the best choice. Reply with just the URL (starting with http:// or https://) or "none" if no suitable official website is found."""
            ...
            if found_urls:
                ...
                for result in results:
                    if found_url in result.get('url', '') or result.get('url', '') in found_url:
    ...
    def print_summary(self):
        ...
        search_found = len([r for r in self.results if r['method'] == 'search_first_result'])
        ...
        print(f"Success rate: {(found/total*100):.1f}%" if total > 0 else "Success rate: 0.0%")
