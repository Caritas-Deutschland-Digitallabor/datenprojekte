...
df = pd.read_csv("/Users/ramius/Desktop/CodeVault/Caritas Datenprojekt/datenprojekte_git/MarkdownConverter/data/csv/PublicInterestAI_Projekte_enriched.csv", sep=";")

all_terms = []
for index, row in df.iterrows(): 
    if pd.notna(row["Art"]):
        terms = [term.strip() for term in row["Art"].split(",") if term.strip()]
...
# 2. CREATE CLUSTERINGS (ALL TERMS ASSIGNED)
...
# 3. CREATE CLUSTER NAMES USING OLLAMA
...
plt.title(f'{best_method} Clustering with Ollama Names')
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('term_clustering_results.png', dpi=300, bbox_inches='tight')
...
results_df.to_csv('term_clustering_results.csv', index=False)
...
summary_df.to_csv('term_clustering_summary.csv', index=False)

print("✅ Complete! Results saved to:")
print("   - term_clustering_results.csv (detailed)")  
print("   - term_clustering_summary.csv (summary)")
print("   - term_clustering_results.png (visualization)")
print(f"\n📊 Summary: {len(clusters)} clusters, {len(unique_terms)} terms, all assigned!")